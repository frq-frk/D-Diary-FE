...
function DiaryEntryForm() {
    ...
    useEffect(() => {
        axios.get(`http://3.26.97.225:5000/v1/dairy/entrybytoday`,
         {
            headers: {
    ...
    const updateEntries = () => {
        axios.put("http://3.26.97.225:5000/v1/user/incrementEntries",{} ,{
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then((response) => {
    ...
    const handleEntrySubmit = (event) => {
        ...
        console.log(text);
        axios.post("http://3.26.97.225:5000/v1/dairy/entry", {
            "entry": text
        },
            {
                headers: {
